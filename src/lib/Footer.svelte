<footer class="footer">
	<div class="container">
		<div class="footer-links">
			<a href="https://classpaddev.github.io/" class="footer-link">Home</a>
			<a href="https://discord.gg/knpcNJTzpd" class="footer-link">Community</a>
			<a href="https://github.com/classpaddev" class="footer-link">GitHub</a>
			<a href="https://classpaddev.github.io/python/" class="footer-link">Python</a>
			<a href="https://classpaddev.github.io/wiki/" class="footer-link">Wiki</a>
			<a href="https://classpaddev.github.io/python/store/" class="footer-link">Python Apps</a>
		</div>
		<div class="social-links">
			<a href="https://classpaddev.github.io/" class="social-link" aria-label="Website">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="20"
					height="20"
					viewBox="0 0 24 24"
					fill="none"
					stroke="currentColor"
					stroke-width="2"
					stroke-linecap="round"
					stroke-linejoin="round"
				>
					<circle cx="12" cy="12" r="10"></circle>
					<line x1="2" y1="12" x2="22" y2="12"></line>
					<path
						d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"
					></path>
				</svg>
			</a>
			<a href="https://discord.gg/knpcNJTzpd" class="social-link" aria-label="Discord">
				<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
					<path
						d="M20.3 4.1c-1.5-.7-3.1-1.2-4.8-1.5-.2-.1-.4 0-.5.1-.1.1-.2.3-.1.5 1.7 3.3 2.1 5.7 2.1 5.7-2.5-1.3-4.3-1.6-6.1-1.6-1.3 0-2.5.3-3.7.9-.1 0-.2 0-.3.1-.1 0-.2-.1-.2-.2 0 0 .4-2.7 2.2-5.9-.1-.1-.3-.1-.4-.1-1.7.3-3.3.8-4.8 1.5-.1 0-.2.1-.2.2-.1.3-.3.9-.2 1.5.1.6.3 1.3.5 1.9.1.2.1.4.1.6-.1.6-.2 1.2-.2 1.8 0 .6.1 1.2.2 1.8 0 .2 0 .4-.1.6-.2.6-.4 1.2-.5 1.9-.1.6 0 1.2.2 1.5.1.1.2.2.4.2 1.5.7 3.1 1.2 4.8 1.5.2.1.4 0 .5-.1.1-.1.2-.3.1-.5-1.7-3.3-2.1-5.7-2.1-5.7 2.5 1.3 4.3 1.6 6.1 1.6 1.3 0 2.5-.3 3.7-.9.1 0 .2 0 .3-.1.1 0 .2.1.2.2 0 0-.4 2.7-2.2 5.9.1.1.3.1.4.1 1.7-.3 3.3-.8 4.8-1.5.1-.1.2-.2.2-.4.3-1.2.5-2.4.5-3.6 0-1.2-.2-2.4-.5-3.6-.1-.3-.2-.6-.4-.8zM8.7 14.7c-.8 0-1.4-.7-1.4-1.5s.6-1.5 1.4-1.5 1.4.7 1.4 1.5-.6 1.5-1.4 1.5zm6.6 0c-.8 0-1.4-.7-1.4-1.5s.6-1.5 1.4-1.5 1.4.7 1.4 1.5-.6 1.5-1.4 1.5z"
					/>
				</svg>
			</a>
			<a href="https://github.com/classpaddev" class="social-link" aria-label="GitHub">
				<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
					<path
						d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"
					/>
				</svg>
			</a>
		</div>
		<p>ClassPad.Dev - Hollyhock for fx-CP400</p>
		<p class="copyright">
			ClassPad.Dev is a community project not affiliated with Casio.<br />
			The programs and other online content on ClassPad.Dev remain the property of their authors and may
			be subject to licenses or copyrights.<br />
			CASIO is a registered trademark of CASIO Computer Co., Ltd. in Japan.
		</p>
	</div>
</footer>
